...
import { axiosData } from "../utils/axiosConfig";
...
function ArticleByID() {
  ...
  useEffect(() => {
    ...
    const getArticle = async () => {
      ...
      try {
        const res = await axiosData.get(`/user-api/article/${id}`);
  ...
  const toggleArticleStatus = async () => {
    ...
    try {
      const res = await axiosData.patch(`/author-api/articles/${id}/status`, {
  ...
  const addComment = async (commentObj) => {
    ...
    let res = await axiosData.put("/user-api/articles", commentObj);
    if (res.status === 200) {
      toast.success(res.data.message);
      setArticle(res.data.payload);
    }
